    ...
    <>
      ...
      <section className="dotgrid border-b-2 border-ink">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
          <Reveal><Kicker className="text-pine">{t("countries.index.kicker", "One planet, sixteen formats")}</Kicker></Reveal>
          <Reveal delay={80}><h1 className="mt-4 max-w-3xl font-display text-4xl font-black leading-[1.05] sm:text-6xl">{t("countries.index.title", "Your resume changes at every border.")}</h1></Reveal>
          <Reveal delay={160}><p className="mt-5 max-w-2xl text-lg text-ink-soft">{t("countries.index.sub", "A photo gets you hired in Germany and rejected in the US. Two pages is thin in Australia and bloated in France. Pick your market; we'll give you the rules.")}</p></Reveal>
        </div>
      </section>
          ...
          {COUNTRIES.map((c, i) => (
            <Reveal key={c.code} delay={(i % 8) * 50}>
              <Link to={`/countries/${c.code}`} className="group flex h-full flex-col border-2 border-ink bg-card p-5 transition-all hover:-translate-y-1 hover:shadow-[6px_6px_0_0_var(--color-ink)]">
                ...
                <h2 className="mt-2 font-display text-xl font-black group-hover:text-pine">{getCountryName(c.code, c.name)}</h2>
                <p className="mt-1 font-mono text-[11px] uppercase tracking-wider text-ink-soft">{c.docName} · {c.pages}</p>
                <p className={`mt-3 text-xs font-bold ${photoTone(c.photo)}`}>{t("countries.photo", "photo")}: {c.photo.toLowerCase()}</p>
                <span className="mt-auto pt-3 text-sm font-bold text-pine">{t("countries.index.readGuide", "Read the guide →")}</span>
              </Link>
            </Reveal>
          ))}
        </div>
...
export function CountryPage() {
  ...
  if (!c) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <h1 className="font-display text-4xl font-black">{t("countries.notFound", "No guide for that market yet.")}</h1>
        <Link to="/countries" className="mt-6 inline-block border-2 border-ink bg-acid px-5 py-3 font-bold">{t("countries.allLink", "All countries →")}</Link>
      </div>
    );
  }
  const countryName = getCountryName(c.code, c.name);
  const facts: [string, string][] = [
    [t("countries.fact.docName", "Document name"), c.docName],
    [t("countries.fact.length", "Length"), c.pages],
    [t("countries.fact.photo", "Photo"), c.photo],
    [t("countries.fact.dateFormat", "Date format"), c.dateFormat],
    [t("countries.fact.phoneFormat", "Phone format"), c.phoneExample],
    [t("countries.fact.paperSize", "Paper size"), c.paper],
  ];
  ...
  return (
    <>
      <Seo
        title={`${countryName} ${c.docName} Format & Rules (2026) — Photo, Pages, ATS | ResumeBuild`}
        description={`How to write a ${c.docName.toLowerCase()} for ${countryName}: ${c.pages}, photo ${c.photo.toLowerCase()}, ${c.dateFormat} dates, local ATS notes and insider tips.`}
        path={`/countries/${c.code}`}
        jsonLd={{ "@context": "https://schema.org", "@type": "Article", headline: `${countryName} ${c.docName} format and rules`, about: { "@type": "Country", name: countryName }, description: c.summary }}
      />
      <section className="dotgrid border-b-2 border-ink">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
          <nav className="font-mono text-xs text-ink-soft">
            <Link to="/countries" className="hover:text-pine">{t("nav.countries", "Countries")}</Link> <span className="mx-1">/</span> <span className="text-ink">{countryName}</span>
          </nav>
          <Reveal><h1 className="mt-5 font-display text-5xl font-black leading-none tracking-tight sm:text-7xl">{countryName} <em className="text-pine">{c.docName}</em></h1></Reveal>
          <Reveal delay={100}><p className="mt-5 max-w-2xl text-lg text-ink-soft">{c.summary}</p></Reveal>
          <Reveal delay={180}>
            <div className="mt-6 flex flex-wrap gap-2">
              <Chip>{c.pages}</Chip>
              <Chip className={photoTone(c.photo)}>{t("countries.photo", "photo")}: {c.photo.toLowerCase()}</Chip>
          ...
          <div className="space-y-10">
            <Reveal>
              <div className="border-2 border-ink bg-card">
                <div className="border-b-2 border-ink bg-ink px-5 py-3"><p className="kicker text-acid">{t("countries.factSheet", "The fact sheet")}</p></div>
            ...
            <div>
              <Reveal><h2 className="font-display text-3xl font-black">{t("countries.howToGetRight", "How to get it right")}</h2></Reveal>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Reveal>
                <div className="h-full border-2 border-pine bg-card p-5">
                  <p className="kicker flex items-center gap-2 text-pine"><Icon name="check" size={14} /> {t("countries.do", "Do")}</p>
              ...
              <Reveal delay={100}>
                <div className="h-full border-2 border-coral bg-card p-5">
                  <p className="kicker flex items-center gap-2 text-coral"><Icon name="x" size={14} /> {t("countries.dont", "Don't")}</p>
                  <ul className="mt-3 space-y-2.5">
            ...
            <Reveal>
              <div className="flex items-start gap-4 border-2 border-ink bg-acid-soft p-5">
                ...
                <div>
                  <p className="font-display text-lg font-black">{t("countries.atsReality", "ATS reality in")} {countryName}</p>
          ...
          <aside className="space-y-5 lg:sticky lg:top-36 lg:self-start">
            <Reveal delay={120}>
              <div className="border-2 border-ink bg-ink p-6 text-paper hs-acid">
                <p className="kicker text-acid">{t("countries.ready", "Ready when you are")}</p>
                <h3 className="mt-2 font-display text-2xl font-black">{t("countries.buildFor", "Build a")} {c.docName.toLowerCase()} {t("countries.thatFits", "that fits")} {countryName}.</h3>
                <p className="mt-2 text-sm text-paper/70">{t("countries.buildSub", "Correct date fields, phone format hints, and ATS-safe templates from the first keystroke.")}</p>
            ...
            <Reveal delay={200}>
              <div className="border-2 border-ink bg-card p-6">
                <p className="kicker text-pine">{t("countries.otherMarkets", "Other markets")}</p>
                  ...
                  {others.map((o) => (
                    <li key={o.code}>
                      <Link to={`/countries/${o.code}`} className="group flex items-center justify-between border border-ink/15 px-3 py-2.5 text-sm font-bold transition-colors hover:border-ink hover:bg-acid-soft">
                        {getCountryName(o.code, o.name)} · {o.docName} <Icon name="arrow" size={14} className="transition-transform group-hover:translate-x-1" />
                      </Link>
                    </li>
                  ))}
                </ul>
