...
export function parseCVToResume(text: string, detectedPageCount?: number): Partial<ResumeData> {
  ...
  // Always default to 1 page layout (gold-standard ATS compliance).
  // A second page is only activated if the user explicitly chooses 2 pages.
  return {
    contact,
    summary,
    experience,
...
export function mergeCVWithResume(parsed: Partial<ResumeData>, existing: ResumeData): ResumeData {
  // Always default to 1 page unless the user has already explicitly chosen 2 pages
  const pageCount: 1 | 2 = existing.pageCount === 2 ? 2 : 1;
