...
export default function CoverLetterDoc({
  ...
  const firstProject = data.projects?.find((p) => p.title);
  ...
  const projectHighlight = firstProject
    ? `Key initiatives include "${firstProject.title}"${firstProject.subtitle ? ` (${firstProject.subtitle})` : ""}${firstProject.bullets?.length ? `, where I ${firstProject.bullets[0].charAt(0).toLowerCase() + firstProject.bullets[0].slice(1).replace(/\.$/, "")}` : ""}.`
    : "";
  ...
  const paragraphs = customBody
    ...
    : [
        ...
        bestMetric || projectHighlight
          ? `Throughout my career, I have consistently prioritized measurable business outcomes and high-quality execution. ${
              projectHighlight ? `${projectHighlight} ` : ""
            }${
              bestMetric
                ? `In my recent role${firstXp?.company ? ` at ${firstXp.company}` : ""}, I ${bestMetric.charAt(0).toLowerCase() + bestMetric.slice(1).replace(/\.$/, "")}. `
                : ""
            }Leveraging core competencies in ${topSkills || "key technical and strategic disciplines"}, I bring hands-on execution and the leadership required to hit the ground running.`
        ...
        <aside className="w-[240px] shrink-0 px-6 py-10 text-white" style={{ background: accent }}>
          ...
          {data.projects && data.projects.length > 0 && (
            <div className="mt-8 border-t border-white/20 pt-6">
              <p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] opacity-80">Key Projects</p>
              <ul className="mt-2 space-y-1 text-[11px] opacity-90">
