...
import { useToast, useResume, useAuth } from "../store/AppStore";
import { apiSaveAtsScan } from "../lib/api";
...
export function CVAnalyzer() {
  ...
  const importToBuilder = useCallback(() => {
    if (parsedResume) {
      ...
      toast("CV successfully imported into live builder!", "ok");
    } else {
  ...
  const runAnalysis = useCallback((text: string, format: "pdf" | "docx" | "txt", pageCount = 1) => {
    try {
      ...
      if (user?.email) {
        const fmtScore = (result.formatting.readableByATS ? 50 : 25) + (result.formatting.isParseable ? 50 : 25);
        apiSaveAtsScan({
          email: user.email,
          score: result.score,
          formattingScore: fmtScore,
          contentScore: result.sections.summary.score + result.sections.experience.score,
          impactScore: result.sections.experience.hasMetrics ? 85 : 60,
